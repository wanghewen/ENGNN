...
class EdgeToNode(DGLDataset):
    ...
    def process(self):
        print("Number of edges for original graph:",
              self.df_train.shape[0] + self.df_val.shape[0] + self.df_test.shape[0])
        ...
        if self.randomize_train_test and not self.randomize_by_node:
            ...
            node_train_index, node_val_index = train_test_split(node_train_index, test_size=0.2, shuffle=True,
                                                                random_state=12)
            ...
            if self.test_portion_index >= 9:
                ...
                test_index, _ = train_test_split(test_index, train_size=(self.test_portion_index - 1) / 8,
                                                 shuffle=False)
            df_train, df_val, df_test = df.iloc[train_index], df.iloc[val_index], df.iloc[test_index]
            self.train_embeds, self.val_embeds, self.test_embeds = embeds[train_index], embeds[val_index], embeds[test_index]
            self.train_labels, self.val_labels, self.test_labels = labels[train_index], labels[val_index], labels[test_index]
        ...
        if scipy.sparse.isspmatrix(self.train_embeds):
            self.train_embeds = self.train_embeds.A
            self.val_embeds = self.val_embeds.A
            self.test_embeds = self.test_embeds.A
        if scipy.sparse.isspmatrix(self.train_labels):
        ...
        #         print(type(self.train_embeds))
        if self.aggregate:
            self.old_train_labels = self.train_labels
            self.old_val_labels = self.val_labels
            self.old_test_labels = self.test_labels
            df_train, self.train_embeds, self.train_labels, train_label_index_mapper = self._process_aggregate(df_train,
                                                                                                     self.train_embeds,
                                                                                                     self.train_labels)
            df_val, self.val_embeds, self.val_labels, val_label_index_mapper = self._process_aggregate(df_val, self.val_embeds,
                                                                                             self.val_labels)
            df_test, self.test_embeds, self.test_labels, test_label_index_mapper = self._process_aggregate(df_test,
                                                                                                 self.test_embeds,
                                                                                                 self.test_labels)
        ...
        edge_labels = torch.cat([self.train_labels, self.val_labels, self.test_labels]).long()
